...
# 同文档票数封顶（2026-09-04 深夜立项标定）：RRF 单一路内同 doc_id 只有前 CAP 个
# chunk 拿全额票，其后作废（chunk 从融合榜除名，仍留在 MMR 候选池末位）。
# 动机：BM25 会把泛词命中的同文件块成堆塞进 top20（P24 实测：svc 文档 12+ 块堆叠，
# 每块各 1 票，把唯一真值文档的单票 0.0164 压到 rank4 之外；P15 同根因）。
# 封顶后堆叠坍塌，MMR 的同文档多样性惩罚才接得住。
# 文件级 hit@k 安全性：doc 前 CAP 票永不作废——doc_id = file_hash 前缀，同 doc 即
# 同文件，文件靠自身 top2 块依然在榜；跨 query 变体的共识每变体各 1 票，不受影响。
# 0 = 关闭（等价经典 RRF）。⚠️ 此值与语料/查询分布强相关，改动需 30 例全量重校准。
# 标定记录（2026-09-04 深夜 A/B，30 例真机）：
#   cap=0（关闭）→ 63.3/73.3/80.0（P24 rank4 miss，P17 hit@1=0）
#   cap=2 → 63.3/73.3/80.0，修 P24 但破 P20（GT 文档自己是 7+7 块堆叠冠军，
#           其最优融合块是 sparse 第 3 位被砍票 0.0325→0.0164）——净零
#   cap=3 → 66.7/76.7/80.0，**严格优于关闭**：P24 修复 + P17 升 top1，
#           en_positive 100/100/100，P20 无回退，负例拒答不变（3/5）→ 定为默认
#   机制：堆叠 ≠ 噪声（P20 的堆就是答案本身）；cap=3 保住 occ≤3 的票、只坍塌
#   12+ 块级深堆，让 MMR 同文档多样性惩罚接得住 P24 类单票真值块。
RRF_DOC_VOTE_CAP = 3

# sparse 阈值闸门（2026-09-04）：sparse 参与 RRF 融合前必须过闸，否则噪声挤进 top4。
# 架构评审 + 30 例实测已证明：RRF 只用名次丢弃分数，sparse 无阈值时 rank1 会压过
# dense rank2 把噪声挤进 top4。噪声有两个来源，闸门必须两管齐下：
#   (1) SPARSE_MIN —— 绝对分数下限。实测稀疏分（-bm25，越大越好）：
#       正例 top1 ∈ [7.88, 67.35]（中位 28.06），负例误召回 ∈ [7.4, 24.5]，
#       两者在 8~24 重叠，单靠分数切不干净（正例最低 7.88 < 负例最高 24.52）。
#       所以 SPARSE_MIN 只能作温和下限（挡掉近乎随机的 <8 分噪声），不是主闸。
#   (2) 判别词命中闸（主闸）—— 语料是英文 Dell 文档，sparse 的可靠信号必须是
#       「查询里的判别词命中了文档」。判别词 = 语料 DF ≤ SPARSE_UBIQUITOUS_RATIO
#       的英文 token（svc_xxx、4-Port、DriveDB…）；DF 超线的是泛词
#       （powerstore 14.6% / io 46% / ha 25%…），命中泛词不算证据。
#       纯中文查询词经 jieba 匹配到中文文档属跨语言结构性误配（N02~N05 的噪声主力），
#       这类召回判别词一个都没命中，直接判负。
SPARSE_MIN = 8.0               # 稀疏分绝对下限（挡 <8 的随机噪声）
SPARSE_REQUIRE_EN_TOKEN = True # 主闸：sparse 结果必须命中 ≥1 个判别词

# 泛词/判别词分界线（2026-09-04 晚 30 例标定，语料 3419 chunk）：
# 英文 token 的语料 DF（含该词的 chunk 数）> TOTAL * RATIO → 泛词，不算证据。
# 标定依据：powerstore DF=500 (14.6%) 必须判泛（否则「PowerStore 量子加密」类查询
# 会误判为有英文证据）；而 drive(390)/from(432)/ha(868)/data(513)/os(942) 等也应判泛。
# 10% 恰好分开：state(232)/xt(233)/db(302)/base(304) 判别，drive(390) 泛。
# ⚠️ 换语料必须重标：阈值是按本语料 DF 分布定的。
SPARSE_UBIQUITOUS_RATIO = 0.10

# 查询级语料证据拒答（零幻觉前置闸，2026-09-04 晚新增，规则 G/F）：
#   规则 G：查询的英文 token 中存在 DF=0（语料零出现）→ 查询点名的专名/术语
#           KB 不覆盖 → 程序层直接拒答，不调 embedding（省延迟）。
#           30 例标定：正例英文 token DF 全 >0（最低 end-of-life=2），零误伤。
#   规则 F：英文判别词集为空 + 查询的中文内容词全部「自身 DF=0 且不在 _ZH_TO_EN
#           词典」→ 中文概念无法建立任何语料证据（跨语言都找不到）→ 拒答。
#           「PowerStore 量子加密」（量子/加密均 DF=0 且无翻译）拒；
#           「PowerStore 数据库卷故障告警」靠 告警→alert 存活，不误伤。
# 两者都只在真实查询路径生效（mock 离线路径跳过）。代价：查询含拼错/生造的
# 英文词会被拒——知识库场景这是正确行为（宁可拒答不可编造）。
...
# 融合策略开关（2026-09-04 实测修正后落地，语义已翻转）：
#   False = dense + sparse 全量 RRF 融合（当前默认）。30 例实测：dense-only 60.0% /
#           sparse-only 63.3% / RRF 融合 73.3%，融合确实反超单路，推翻了 9-3 的
#           「sparse 是净负债」结论（那结论只对纯中文查询成立）。
#           融合的前提是 sparse 先过 SPARSE_MIN + 英文词命中两道闸门（见上），
#           否则无阈值噪声会挤进 top4。
#   True  = dense 主力 + sparse 兜底（逃生舱）：dense 一旦有通过 DENSE_MIN 的候选就
#           只信 dense；仅当 dense 空手时才回退 sparse（救英文词/命令精确匹配）。
#           用于评估对照 / 怀疑 sparse 噪声时一键降级。
# 名字保留 SPARSE_FALLBACK_ONLY 是为兼容旧引用；语义以本注释为准。
SPARSE_FALLBACK_ONLY = False
